...
.auth-header p { font-size: 12px; color: var(--ink-muted); font-family: var(--font-mono); text-transform: uppercase; letter-spacing: 0.06em; margin: 0; word-break: break-word; }
.auth-error {
...
.auth-success {
  ...
  padding: 10px 14px; border-radius: 2px; font-size: 13px; margin-bottom: 18px; font-weight: 600; line-height: 1.5;
}
...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setIsOtpSent(true);
      setMessage('An OTP has been sent to your college email.');
    } catch (err) {
  ...
  // Resend OTP without leaving the verification step
  const handleResend = async () => {
    ...
    try {
      await API.post('/auth/register', { name: name.trim(), email, password });
      setMessage('A new OTP has been sent to your college email.');
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.message || 'Could not resend the OTP.');
    } finally {
      setResending(false);
    }
  };
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await API.post('/auth/verify-otp', { email, otp: otp.trim() });
      localStorage.setItem('token', response.data.token);
      navigate('/');
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.message || 'Invalid or expired OTP. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="auth-root">
      ...
      <div className="auth-card">
        {isOtpSent && (
          <button type="button" className="auth-back" onClick={handleEditDetails}>
        ...
        <div className="auth-header">
          ...
          <p>{isOtpSent ? `Code sent to ${email}` : 'Exclusive to NIT Kurukshetra students'}</p>
        </div>
